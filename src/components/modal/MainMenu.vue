...
const mainMenuItem = [
  ...
  {
    title: 'barang',
    child: [{ icon: box, label: 'daftar barang', action: () => { } }],
  },
  {
    title: 'akun',
    ...
    <ion-content>
      <div class="modal-main-menu__inner">
        <div v-for="(item, index) in mainMenuItem" :key="index + 1">
          <h3>{{ item.title }}</h3>
          <ion-list lines="none">
            <ion-item class="ion-activatable" v-for="(subItem, subIndex) in item.child" :key="subIndex + 1"
              @click="subItem.action">
              <custom-icon :svg-icon="subItem.icon" width="20"></custom-icon>
